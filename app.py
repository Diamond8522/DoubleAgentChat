...
# This safely gets the key from Streamlit's secrets.toml
try:
    groq_client = Groq(api_key=st.secrets["GROQ_API_KEY"])
except Exception as e:
    # If the key is missing or incorrect, stop and show a clear error
    st.error("FATAL ERROR: Groq API key not found or incorrect in Streamlit Secrets. Please set your GROQ_API_KEY.")
    st.stop()
...
def generate_agent_response(persona_name, system_prompt, history):
    ...
    # Parameters to discourage repetition and add variety (The Fix!)
    TEMP = 0.7 
    FREQ_PENALTY = 0.5 
...
if "messages" not in st.session_state:
    ...
    st.session_state.messages.append({"role": "Violet", "content": "Welcome to the Lab! We're up and running on Groq now. Resilience, right? What challenging project are we tackling first?"})
...
# --- User Input and Agent Orchestration (The Magic) ---
...
if prompt := st.chat_input("Ask Violet and Storm anything..."):
    ...
    tasks = {
        "Violet": (VIOLET_SYSTEM_PROMPT, "Violet is drafting her tech-savvy reply..."),
        "Storm": (STORM_SYSTEM_PROMPT, "Storm is abstractly thinking about optimization...")
    }

    # Execute both agents concurrently using ThreadPoolExecutor for speed
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        # Collect results in order (Violet first, then Storm)
